...
## Retention Policy
By default, audit logs older than 120 days are automatically pruned daily. You can configure the retention period using the `SOURCEBOT_EE_AUDIT_RETENTION_DAYS` [environment variable](/docs/configuration/environment-variables). Set it to `0` to disable automatic pruning and retain logs indefinitely.
...
Audit logs are retained for 90 days by default, configurable with `SOURCEBOT_EE_AUDIT_RETENTION_DAYS`.

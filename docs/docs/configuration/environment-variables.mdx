...
| `AUTH_SESSION_MAX_AGE_SECONDS` | `86400` (1 day) | <p>Relative time from now in seconds when to expire the session.</p> |
| `AUTH_SESSION_UPDATE_AGE_SECONDS` | `21600` (6 hours) | <p>How often the session should be updated in seconds. If set to `0`, session is updated every time.</p> |
...
| `SOURCEBOT_EE_AUDIT_RETENTION_DAYS` | `120` | <p>The number of days to retain audit logs. Audit log records older than this will be automatically pruned daily. Set to `0` to disable pruning and retain logs indefinitely.</p> |

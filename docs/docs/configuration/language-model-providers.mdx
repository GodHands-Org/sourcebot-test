...
<Note>The `credentials` parameter here expects an object with either an `env` key (the name of an environment variable) or a `googleCloudSecret` key (a Google Cloud secret resource name).</Note>

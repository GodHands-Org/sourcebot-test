...
Retrieves file contents for one or more files by accepting a JSON request body validated against the files request schema. Returns a JSON response or a service error if the request is invalid or the underlying operation fails.
